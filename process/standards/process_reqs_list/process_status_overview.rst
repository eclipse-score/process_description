...
compliance status, and work product completion status in a single overview.
...
.. raw:: html
   ...
   document.addEventListener('DOMContentLoaded', function() {
     // Merge columns 0-2 of the combined status table and vertically center them.
     ...
     // Mark rows that start a new process-area group (col 0 has content)
     // with a CSS class for the group separator, skipping the very first group.
     var firstGroup = true;
     ...
     // Process in reverse column order so that removing higher-index cells
     // does not shift the indices of lower-index cells in later passes.
